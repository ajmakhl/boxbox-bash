...
import { rankColors, rankBorderColors } from '@modules';
import { service, notesPageStore, notesPageActions } from '@services/index';
import { GeneralNotesCard } from './GeneralNotesCard';

export function NotesPlayerList() {
  const { players, order } = service.useSnapshot();
  ...
  const handleSelectPlayer = (playerId: string) => {
    notesPageActions.selectPlayer(playerId);
  };
  ...
  return (
    <div className='flex flex-col h-full min-h-0 rounded-lg border border-slate-500 overflow-hidden'>
      <div className='shrink-0 px-4 py-3 border-b border-slate-500'>
        ...
        <p className='text-sm text-gray-400'>Select a player to tag in notes</p>
      </div>
      <ScrollArea.Root className='flex-1 min-h-0 overflow-hidden'>
        <ScrollArea.Viewport className='h-full w-full overflow-auto p-3'>
          {/* General Notes Card */}
          <GeneralNotesCard
            isSelected={isGeneralNotesSelected}
            onSelect={() => handleSelectPlayer('general')}
          />
          ...
          <div className='space-y-2'>
            {filteredOrder.map((playerId) => {
              ...
              const rankBorder =
                rankBorderColors[player.rank as IRank] ?? 'border-gray-500';
              return (
                <button
                  key={playerId}
                  onClick={() => handleSelectPlayer(playerId)}
                  className={`w-full text-left px-4 py-3 rounded-lg transition-colors flex items-center gap-3 border cursor-pointer ${
                    selectedPlayerId === playerId
                      ? `${rankBorder} text-white`
